...
def generate_synthetic_data(district_info):
    ...
    df = pd.DataFrame(
        {
            "datetime": date_range,
            "temperature_2m": fetch_weather_data["temperature_2m"],
...
class RenewableEnergyModels:
    ...
    def predict_district_potential(self, district_name, district_data):
        ...
        daily_energy = daily_energy / 1000
...
def predict_with_forecast_data(forecast_data, district_name):
    ...
    print(f"Wind Energy Potential: {results['total_potential']['wind']:.2f} Wh/m²/day")
    print(
        f"Solar Energy Potential: {results['total_potential']['solar']:.2f} Wh/m²/day"
    )

    if results["coastal"]:
        print(
            f"Ocean Energy Potential: {results['total_potential']['ocean']:.2f} Wh/m²/day"
        )

    print(
        f"Total Energy Potential: {sum(results['total_potential'].values()):.2f} Wh/m²/day"
    )
...
def get_forecast_and_predict():
    ...
    plt.xlabel("Date")
    plt.ylabel("Energy Potential (Wh/m²/day)")
